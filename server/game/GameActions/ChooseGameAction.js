...
class ChooseGameAction extends GameAction {
    ...
    createEvent(context) {
        return this.event('onChoose', {}, (event) => {
            ...
            const cancelText =
                this.cancelText instanceof Function
                    ? this.cancelText(tempContext)
                    : this.cancelText;
            const cancelMessage =
                this.cancelMessage instanceof Function
                    ? this.cancelMessage(tempContext)
                    : this.cancelMessage;

            context.game.queueStep(
                new AbilityChoicePrompt({
                    game: context.game,
                    context,
                    choosingPlayer,
                    title,
