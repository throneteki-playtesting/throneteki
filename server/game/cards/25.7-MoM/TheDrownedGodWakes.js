        ...
        this.reaction({
            when: {
                onDominanceDetermined: (event) =>
                    this.controller === event.winner && event.difference >= 10
            },
            message:
                '{player} plays {source} to have each player choose a character they control, and kill the others',
            target: {
                choosingPlayer: 'each',
                ifAble: true,
            ...
            handler: (context) => {
                ...
                let characters = this.game.filterCardsInPlay(
                    (card) => !targets.includes(card) && card.getType() === 'character'
                );
...
export default TheDrownedGodWakes;
