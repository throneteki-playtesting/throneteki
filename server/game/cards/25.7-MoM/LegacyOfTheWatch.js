...
class LegacyOfTheWatch extends DrawCard {
        ...
        this.reaction({
            when: {
                afterChallenge: (event) =>
                    ...
                    this.satisfiableBonuses(event.challenge).length > 0
            },
            handler: (context) => {
    ...
    satisfiableBonuses(challenge) {
        ...
        if (
            participants.filter((card) => card.hasTrait('Ranger')).length >= 2 &&
            this.controller.canGainFactionPower()
        ) {
            satisfiable.push('power');
        }
        if (
            participants.filter((card) => card.hasTrait('Steward')).length >= 2 &&
            this.controller.canDraw()
        ) {
            satisfiable.push('draw');
        }
        if (
            participants.filter((card) => card.hasTrait('Builder')).length >= 2 &&
            this.game.anyCardsInPlay((card) => this.canStandLocation(card))
        ) {
